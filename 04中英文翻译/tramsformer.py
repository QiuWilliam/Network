...
def train(model, dataloader, optimizer, epoch, max_epochs):
    model.train()
    total_loss = 0
...
for epoch in range(1, epochs + 1):
    avg_loss = train(model, train_loader, optimizer, epoch, epochs)
    avg_bleu_score = validate(model, dev_loader, tokenizer)
    print(f"Epoch [{epoch}/{epochs}] Average Loss: {avg_loss:.4f}, Average BLEU Score: {avg_bleu_score:.2f}")
